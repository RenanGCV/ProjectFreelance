...
const contactSchema = z.object({
  ...
  contato: z.string().min(5, "Informe um contato valido"),
...
export function ContactForm() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4 rounded-2xl border border-white/15 bg-black/10 p-6">
      ...
      <label className="grid gap-1">
        ...
        <textarea
          ...
          rows={4}
          className="rounded-xl border border-white/20 bg-slate-950/40 px-4 py-3 text-slate-100 placeholder:text-slate-400 focus:border-brand-primary focus:outline-none"
          placeholder="Me conte seu objetivo e o que voce precisa"
        />
      ...
      <button
        ...
        className="rounded-xl bg-brand-primary px-5 py-3 font-semibold text-slate-950 transition hover:bg-brand-accent disabled:opacity-60"
      >
        {isSubmitting ? "Enviando..." : "Enviar e falar no WhatsApp"}
      </button>

      {isSubmitSuccessful ? (
        <p className="text-sm text-emerald-300">Perfeito. Agora so confirmar os detalhes no WhatsApp.</p>
      ) : null}
    </form>
  );
}
